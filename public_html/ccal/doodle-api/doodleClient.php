 ...
 ****************************************************************/
     ...
     * (access token) and oauth_token_secret.
     */
    ...
    function extract_id($url)
    {
        ...
        // check that user typed correct host (if http or https input)
        if(isset($url['host']))
        {
            // check that user typed correct host
            if($url['host'] !="www.doodle.com" && $url['host'] !="doodle.com")
                apologize("Sorry, an invalid doodle link as been entered. Please try again.");
        
            // check for path, if exists extract id
            else if(isset($url['path']))
                $id = substr($url['path'], 1, 16);
        
            else
                apologize("Sorry, an error occurred. Please try again.");
        }
        
        // if this condition entered, http(s) not entered
        else
        {
            // explode and extract id from string
            $result = explode("/", $url['path'], 2);
